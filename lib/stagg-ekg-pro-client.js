...
class StaggEKGProClient {
    ...
    commandAsync (cmd) {
        ...
        // Keep the entire request, including the response body, below
        // Homebridge's three-second characteristic warning threshold.
        const signal = AbortSignal.timeout(2500);
        const promise = fetch(`${this.cliUrl}?cmd=${encoded}`, { signal })
            .then(async res => {
                if (!res.ok) {
            ...
            .catch(err => {
                // A timeout while reading the body can surface as AbortError;
                // retain the timeout reason for a useful connection diagnostic.
